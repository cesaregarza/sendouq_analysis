...
def get_most_influential_matches(
    ...
    if not hasattr(engine, 'tournament_influence_') or engine.tournament_influence_ is None:
        raise ValueError("Engine must have computed tournament influences. Run rank_players() first.")
    ...
    matches_with_influence = matches_df.join(
        ...
        on="tournament_id", 
        how="left"
    ).fill_null(1.0)  # Default influence for unseen tournaments
    ...
    matches_with_influence = matches_with_influence.with_columns(
        (
            (int(engine.now.timestamp()) - pl.col("event_ts").cast(pl.Float64)) / 86400.0
        ).mul(-engine.decay_rate).exp().alias("time_decay")
    )
    
    # Calculate match weight (time decay * tournament strength^beta)
    matches_with_influence = matches_with_influence.with_columns(
        [
            (pl.col("time_decay") * (pl.col("tournament_influence") ** engine.beta)).alias("match_weight"),
    ...
    wins = (
        ...
        .filter(pl.col("loser_team_id").is_not_null())  # Valid matches only
    )
    ...
    losses = (
        ...
        .filter(pl.col("winner_team_id").is_not_null())  # Valid matches only
    )
    ...
    if not wins.is_empty():
        ...
        wins = (
            wins
            .sort("match_weight", descending=True)
            .with_columns(
                pl.arange(1, wins.height + 1).alias("influence_rank")
            )
            .select([
                "influence_rank",
                "tournament_id",
