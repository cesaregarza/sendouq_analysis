...
class DataValidator:
    ...
    def validate_with_parser(self, data: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            if (
                tables.get("players") is not None
                and tables.get("teams") is not None
            ):
                self._validate_roster_integrity(
                    tables["players"], tables["teams"]
                )
    ...
    def _validate_roster_integrity(
        self, players_df: pl.DataFrame, teams_df: pl.DataFrame
    ):
