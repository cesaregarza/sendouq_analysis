...
class SyntheticPlayer:
    ...
    def get_performance(self, rng: Optional[np.random.Generator] = None) -> float:
        """
...
class PlayerGenerator:
    ...
    def generate_players(
        ...
        skills = self._generate_skills(n_players, skill_distribution, skill_params)
        players = []
    ...
    def generate_elite_players(self, n_players: int, base_skill: float = 2.0) -> List[SyntheticPlayer]:
        """
        ...
        for i in range(n_players):
            player = SyntheticPlayer(
                ...
                true_skill=skills[i],
                skill_variance=self.rng.uniform(0.05, 0.15),  # Lower variance for elite
                team_affinity=self._generate_team_affinity()
            )
            players.append(player)
            self._next_user_id += 1
    ...
    def _generate_skills(
        ...
        if distribution == "normal":
            ...
            return params["high"] - self.rng.exponential(params["scale"], n_players)
        else:
