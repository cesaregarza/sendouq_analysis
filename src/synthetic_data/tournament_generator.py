...
class TournamentGenerator:
    ...
    def generate_multi_stage_tournament(
        ...
        for config in stages_config:
            ...
            if format == TournamentFormat.SWISS:
                ...
                stage = self._generate_round_robin_stage(current_teams, **config)
            elif format == TournamentFormat.SINGLE_ELIMINATION:
                stage = self._generate_single_elimination_stage(current_teams, **config)
            elif format == TournamentFormat.DOUBLE_ELIMINATION:
                stage = self._generate_double_elimination_stage(current_teams, **config)
            elif format == TournamentFormat.GROUP_STAGE:
            ...
            if "advance_count" in config:
                current_teams = self._get_advancing_teams(stage, config["advance_count"])
    ...
    def _form_teams(self, players: List[SyntheticPlayer], team_size: int) -> List[Team]:
        """Form teams from player pool."""
        # Shuffle players
    ...
    def _generate_single_elimination_stage(
        ...
        **kwargs
        ...
        if seeded:
            sorted_teams = sorted(teams, key=lambda t: t.avg_skill, reverse=True)
            ...
            if seeded and round_num == 1:
                ...
                for i in range(n // 2):
                    ...
                    if self.rng.random() < 0.7:  # 70% chance for better team
                        winner = current_teams[i] if current_teams[i].avg_skill > current_teams[n - 1 - i].avg_skill else current_teams[n - 1 - i]
                    else:
                        winner = current_teams[n - 1 - i] if current_teams[i].avg_skill > current_teams[n - 1 - i].avg_skill else current_teams[i]
                ...
                for i in range(0, len(current_teams), 2):
                    if i + 1 < len(current_teams):
                        ...
                        if self.rng.random() < 0.6:
                            winner = current_teams[i] if current_teams[i].avg_skill > current_teams[i + 1].avg_skill else current_teams[i + 1]
                        else:
                            winner = current_teams[i + 1] if current_teams[i].avg_skill > current_teams[i + 1].avg_skill else current_teams[i]
                        next_round_teams.append(winner)
                    else:
                        # Bye
    ...
    def _get_advancing_teams(self, stage: TournamentStage, advance_count: int) -> List[Team]:
    ...
    def _assign_match_timestamps(self, tournament: Tournament, start_date: datetime):
