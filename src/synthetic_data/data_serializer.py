...
class DataSerializer:
        ...
        tournament_data = {
            "tournament": {
                ...
                "ctx": {
                    ...
                    "startTime": tournament.start_date.isoformat() if tournament.start_date else None,
                    "teams": []
                }
            }
        }
    ...
    def _serialize_stage(
        ...
        stage_data = {
            ...
            "settings": {
                ...
                "matchesChildCount": 0,
                "groupCount": 4 if stage.format == TournamentFormat.GROUP_STAGE else 0,
                "roundRobinMode": "simple" if stage.format == TournamentFormat.ROUND_ROBIN else None,
    ...
    def _serialize_match(
        ...
        match_data = {
            ...
            "number": match_number,
            "status": status,
            "lastGameFinishedAt": match.timestamp.isoformat() if match.timestamp and match.winner else None,
            "createdAt": match.timestamp.isoformat() if match.timestamp else datetime.now().isoformat(),
            "opponent1": None,
            "opponent2": None
        }
        ...
        if match.team_a:
            match_data["opponent1"] = {
                ...
                "position": match.team_a.seed if match.team_a.seed else match_number * 2 - 1,
                "score": match.score_a,
                "result": "win" if match.winner == match.team_a else ("loss" if match.winner else None)
            }
        ...
        if match.team_b:
            match_data["opponent2"] = {
                ...
                "position": match.team_b.seed if match.team_b.seed else match_number * 2,
                "score": match.score_b,
                "result": "win" if match.winner == match.team_b else ("loss" if match.winner else None)
            }
    ...
    def to_json_file(
        ...
        data = self.serialize_tournaments(tournaments, simulate_matches, match_simulator)
