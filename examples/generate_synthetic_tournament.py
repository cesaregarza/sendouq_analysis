...
def generate_competitive_tournament():
    ...
    player_pool = player_gen.create_player_pool_with_categories(
        n_elite=20, n_competitive=80, n_casual=100
    )
...
def generate_tournament_series():
    ...
    base_players = player_gen.create_player_pool_with_categories(
        n_elite=30, n_competitive=120, n_casual=250
    )
    ...
    for i, format in enumerate(formats):
        ...
        participating_players = rng.sample(
            all_players, k=min(64, len(all_players))
        )
...
if __name__ == "__main__":
    ...
    save_tournaments_to_file(
        competitive_data, "synthetic_competitive_tournament.json"
    )
